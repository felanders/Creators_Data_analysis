...
```{r fig_2}
#the  Hmisc package must be installed for this to run!
df_cal_long %>% 
ggplot(aes(x=round)) +
...
df_table %>% 
  ...
  left_join(df_table %>% filter(Treat == "Nonrival Treatment"), by = c("round")) %>% 
  ungroup() %>% 
  select(round, A.x, B.x, C.x, N.x, A.y, B.y, C.y, N.y) %>% 
  kable(col.names=c("Round", "A (Do. N)","B (Buy)","C (Submit)", "N", "A (Do. N)","B (Buy)","C (Submit)", "N")) %>% 
  add_header_above(c(" " = 1, "Rival" = 4, "Nonrival" = 4)) %>% 
  add_header_above(c(" " = 1, "Treatment" = 8)) %>% 
...
df_table %>% 
  ...
  select(round, A.x, B.x, C.x, N.x, A.y, B.y, C.y, N.y) %>% 
  kable(col.names=c("Round", "A (Do. N)","B (Buy)","C (Submit)", "N", "A (Do. N)","B (Buy)","C (Submit)", "N")) %>% 
  add_header_above(c(" " = 1, "Supplementary" = 4, "Nonrival" = 4)) %>% 
  add_header_above(c(" " = 1, "Treatment" = 8)) %>% 
  kable_material(c("striped", "hover"))
```
